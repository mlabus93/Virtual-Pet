...
typedef struct {
	...
	float4 normal; // xyz - surface tension normal, w - prenormalized length
	float density;
	float pressure;
...
#define FLUVIO_GRID_BUCKET_SIZE 16
...
#define FluvioParticleIsAlive(particleIndex) ((particleIndex) < fluvio_Count.z && fluvio_Particle[(particleIndex)].lifetime.x > 0.0f)

// Returns true if the plugin should be updated for this particle
#define FluvioShouldUpdatePlugin(particleIndex) ((particleIndex) < fluvio_Count.z && FluvioParticleIsAlive((particleIndex)) && particleIndex < fluvio_Count.z && (fluvio_IncludeFluidGroup == 1 || fluvio_PluginFluidID == fluvio_Particle[(particleIndex)].id.x))
...
#define solverData_GetDensity(particleIndex) (fluvio_Particle[(particleIndex)].density)
...
#define solverData_GetPressure(particleIndex) (fluvio_Particle[(particleIndex)].pressure)
